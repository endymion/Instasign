...
protected:
  ...
  size_t _CA_cert_len;  // 0 => PEM (strlen); >0 => DER length
  size_t _cert_len;
...
public:
  ...
  // DER (binary) credentials — required when mbedTLS is built without PEM_PARSE_C.
  void setCACertDer(const uint8_t *rootCA, size_t len);
  void setCertificateDer(const uint8_t *client_ca, size_t len);
  void setPrivateKeyDer(const uint8_t *private_key, size_t len);
