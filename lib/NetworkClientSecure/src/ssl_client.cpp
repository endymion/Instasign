...
// Tasmota/Arduino frameworks often omit PSK in mbedtls; TLS cert auth still works.
#endif
...
int start_ssl_client(
  ...
  size_t rootCA_len, size_t cli_cert_len, size_t cli_key_len
) {
  int ret;
  int enable = 1;
...
#define ROE(x, msg)                                   \
  ...
  if (insecure) {
    ...
    // len>0: DER (or raw) with explicit length; len==0: PEM via strlen+1
    size_t caLen = rootCA_len ? rootCA_len : (strlen(rootCABuff) + 1);
    ret = rootCA_len
            ? mbedtls_x509_crt_parse_der(&ssl_client->ca_cert, (const unsigned char *)rootCABuff, caLen)
            : mbedtls_x509_crt_parse(&ssl_client->ca_cert, (const unsigned char *)rootCABuff, caLen);
  ...
  if (!insecure && cli_cert != NULL && cli_key != NULL) {
    ...
    size_t certLen = cli_cert_len ? cli_cert_len : (strlen(cli_cert) + 1);
    ret = cli_cert_len
            ? mbedtls_x509_crt_parse_der(&ssl_client->client_cert, (const unsigned char *)cli_cert, certLen)
            : mbedtls_x509_crt_parse(&ssl_client->client_cert, (const unsigned char *)cli_cert, certLen);
    if (ret < 0) {
    ...
    size_t keyLen = cli_key_len ? cli_key_len : (strlen(cli_key) + 1);
#if MBEDTLS_VERSION_MAJOR >= 4
    ret = mbedtls_pk_parse_key(&ssl_client->client_key, (const unsigned char *)cli_key, keyLen, NULL, 0);
#else
    mbedtls_ctr_drbg_context ctr_drbg;
    mbedtls_ctr_drbg_init(&ctr_drbg);
    ret = mbedtls_pk_parse_key(&ssl_client->client_key, (const unsigned char *)cli_key, keyLen, NULL, 0, mbedtls_ctr_drbg_random, &ctr_drbg);
