...
int NetworkClientSecure::connect(IPAddress ip, uint16_t port, const char *host, const char *CA_cert, const char *cert, const char *private_key) {
  // When callers pass the member pointers set via setCACert*, include DER lengths.
  size_t caLen = (CA_cert == _CA_cert) ? _CA_cert_len : 0;
  size_t certLen = (cert == _cert) ? _cert_len : 0;
  size_t keyLen = (private_key == _private_key) ? _private_key_len : 0;
  int ret = start_ssl_client(
    sslclient.get(), ip, port, host, _timeout, CA_cert, _use_ca_bundle, cert, private_key, NULL, NULL, _use_insecure, _alpn_protos, caLen, certLen, keyLen
  );
...
void NetworkClientSecure::setCACertDer(const uint8_t *rootCA, size_t len) {
  if (_ca_cert_free && _CA_cert) {
    free((void *)_CA_cert);
    _ca_cert_free = false;
  }
  _CA_cert = (const char *)rootCA;
  _CA_cert_len = len;
  _use_insecure = false;
}
...
void NetworkClientSecure::setCertificateDer(const uint8_t *client_ca, size_t len) {
  if (_cert_free && _cert) {
    free((void *)_cert);
    _cert_free = false;
  }
  _cert = (const char *)client_ca;
  _cert_len = len;
}
...
void NetworkClientSecure::setPrivateKeyDer(const uint8_t *private_key, size_t len) {
  if (_private_key_free && _private_key) {
    free((void *)_private_key);
    _private_key_free = false;
  }
  _private_key = (const char *)private_key;
  _private_key_len = len;
}
